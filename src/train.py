...
# from utils import save_model, print_monitor_info, monitor_model_weights
...
def main():
    ...
    # 逻辑: 当验证集指标 (ROUGE-L) 不再上涨时，降低学习率
    # =========================================================
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
        optimizer, 
        mode='max',      # 我们希望 ROUGE 越高越好
        factor=0.5,      # 触发时，学习率乘以 0.5
        patience=2,      # 容忍 2 个 epoch 不涨分
        verbose=True,    # 打印日志
        min_lr=1e-7      # 学习率下限
    )
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        print(f"ROUGE-1: {val_scores['r1_f']:.4f} | ROUGE-L: {current_rouge_l:.4f}")

        # =========================================================
        # [Step Scheduler] 更新学习率
        # =========================================================
        scheduler.step(current_rouge_l)

        # 调试调度器状态
        current_lrs = [f"{pg['lr']:.2e}" for pg in optimizer.param_groups]
        print(f"[Scheduler] ROUGE-L={current_rouge_l:.6f}, best={scheduler.best:.6f}, "
            f"bad_epochs={scheduler.num_bad_epochs}, LRs=[{', '.join(current_lrs)}]")
